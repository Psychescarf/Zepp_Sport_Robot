...
def get_code(location):
    code_pattern = re.compile("(?<=access=).*?(?=&)") # 利用表达式检索出登录code相关部分
    code = code_pattern.findall(location)[0] # 赋值到code变量
    return code
...
def login(user, passwd):
    ...
    try:
        ...
        login_token = res["token_info"]["login_token"] # 从返回数据中解码获取token
        userid = res["token_info"]["user_id"] # 从返回数据中解码获取用户ID
        print(f'账号{user}获取token成功')
...
# 修改步数
def change_steps(user, userid, app_token, step=None):
    ...
    finddate = re.compile(r'.*?date%22%3A%22(.*?)%22%2C%22data.*?') # 利用表达式检索出日期相关部分
    findstep = re.compile(r'.*?ttl%5C%22%3A(.*?)%2C%5C%22dis.*?') # 利用表达式检索出步数相关部分
    data_json = re.sub(finddate.findall(data_json)[0], today, str(data_json)) # 添加日期信息到数据中
    data_json = re.sub(findstep.findall(data_json)[0], step, str(data_json)) # 添加步数信息到数据中
    url = f'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={get_timestamp()}' # 定义请求url
    head = {
        "apptoken": app_token,
        "Content-Type": "application/x-www-form-urlencoded"
    }
    data = f'userid={userid}&last_sync_data_time=1597306380&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={data_json}' # 构造请求数据
    try:
        response = requests.post(url, data=data, headers=head).json() # 发送步数更新请求并获取请求结果
        _type = f"账号{user}*******"
        print(f"{_type}: 修改步数{step} " + response['message']) # 输出请求结果信息
        return True
    except Exception as err:
        print(err)
        return None
...
if __name__ == "__main__":
    ...
    for i in account:
        ...
        login_token, userid = login(i[0], i[1]) # 登录
        if not login_token:
            print('登录失败')
            continue

        app_token = get_app_token(login_token) # 获取app_token
        if not app_token:
            continue

        result = change_steps(i[0], userid, app_token, str(step)) # 修改步数并获取返回结果
        if not result:
            continue
